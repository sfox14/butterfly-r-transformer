...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, butterfly=False):
        super(PositionwiseFeedForward, self).__init__()
        d_ff = d_model * 4
        if butterfly:
            self.w_1 = Butterfly(d_model, d_ff, bias=False, complex=False, init='ortho', nblocks=2)
            self.w_2 = Butterfly(d_ff, d_model, bias=False, complex=False, init='ortho', nblocks=2)
        else:
...
class MHPooling(nn.Module):
    def __init__(self, d_model, h, dropout=0.1, butterfly=False):
        ...
        self.h = h
        if butterfly:
            self.linears = clones(Butterfly(d_model, d_model, bias=True, complex=False, init='ortho', nblocks=2), 4)
        else:
...
class LocalRNN(nn.Module):
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout, butterfly):
        ...
        self.ksize = ksize
        if rnn_type == 'GRU':
            self.rnn = nn.GRU(output_dim, output_dim, batch_first=True, bias=False)
        elif rnn_type == 'LSTM':
            self.rnn = nn.LSTM(output_dim, output_dim, batch_first=True, bias=False)
        elif rnn_type == 'CUSTOM':
            self.rnn = LSTMLayer(output_dim, output_dim, butterfly=butterfly, bias=False)
        else:
            self.rnn = nn.RNN(output_dim, output_dim, batch_first=True, bias=False)

        if butterfly:
            self.output = nn.Sequential(Butterfly(output_dim, output_dim, bias=True, complex=False, init='ortho', nblocks=2), nn.ReLU())
        else:
...
class LocalRNNLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout, butterfly):
        ...
        self.local_rnn = LocalRNN(input_dim, output_dim, rnn_type, ksize, dropout, butterfly)
...
class Block(nn.Module):
    ...
    One Block
    """
    def __init__(self, input_dim, output_dim, rnn_type, ksize, N, h, dropout, butterfly):
        super(Block, self).__init__()
        self.layers = clones(
            LocalRNNLayer(input_dim, output_dim, rnn_type, ksize, dropout, butterfly), N)
        self.connections = clones(SublayerConnection(output_dim, dropout), 2)
        self.pooling = MHPooling(input_dim, h, dropout, butterfly)
        self.feed_forward = PositionwiseFeedForward(input_dim, dropout, butterfly)
...
class RTransformer(nn.Module):
    ...
    The overal model
    """
    def __init__(self, d_model, rnn_type, ksize, n_level, n, h, dropout, butterfly):
        ...
        for i in range(n_level):
            layers.append(
                Block(d_model, d_model, rnn_type, ksize, N=N, h=h, dropout=dropout, butterfly=butterfly))
